pipeline {
   ...
   parameters {
  string defaultValue: '0.0.1', name: 'version'
}
    stages {
        stage('git-pull'){
            steps {
        ...
        stage('push-artifact'){
            steps { 
             sh 'aws s3 ls'
                ...
                #unzip awscliv2.zip
                #sudo ./aws/install
                 sudo mv /home/ubuntu/workspace/jenkins/target/studentapp-2.2-SNAPSHOT.war /home/ubuntu/student-${version}.war
                 aws s3 cp /home/ubuntu/student-${version}.war s3://shubham.goutam
                 '''
           }
        }
        stage('tomcat-deploy'){
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'tomcat-server', keyFileVariable: 'new', usernameVariable: 'ubuntu')]) {
                    ...
                    sudo apt-get install unzip -y
                        ...
                        #unzip awscliv2.zip
                        #sudo ./aws/install
                        aws s3 cp s3://shubham.goutam/student-${version}.war .
